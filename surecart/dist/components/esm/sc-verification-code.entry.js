...
const ScVerificationCode = class {
    ...
    render() {
        return (h("div", { key: 'b38356eddf05e5abf1d2aaa3ce6d284d4b6cd0c5', class: "sc-verification-code" }, Array.from({ length: this.total }).map((_, index) => (h(Fragment, null, index === Math.floor(this.total / 2) && (h("span", { class: "sc-verification-code__separator", "aria-hidden": "true" }, "\u2014")), h("input", { key: index, id: `code-input-${index}`, value: !!this.codes[index] ? this.codes[index] : '', onInput: e => this.handleInput(e, index), onKeyDown: e => this.handleKeyDown(e, index), onFocus: e => this.handleFocus(e), autocomplete: "one-time-code", inputmode: "numeric", pattern: "[0-9]*", autofocus: index === 0, required: true, "aria-label": wp.i18n.__(`Verification code ${index + 1} of ${this.total}`, 'surecart') })))), h("button", { key: '8ba157a662f591206eac34bba82e7bde3387d545', type: "submit", class: "visually-hidden", onClick: () => this.onChange(this.codes.join('')) }, wp.i18n.__('Submit', 'surecart'))));
    }
