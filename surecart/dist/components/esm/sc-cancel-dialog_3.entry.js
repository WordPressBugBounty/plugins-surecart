...
const ScCancelDialog = class {
    ...
    render() {
        return (h("sc-dialog", { key: 'cd046286aedbd1f52fefc4c1e60dac9c3fdca4f9', style: {
                '--width': this.step === 'survey' ? '675px' : '500px',
                '--body-spacing': 'var(--sc-spacing-xxx-large)',
            }, noHeader: true, open: this.open, onScRequestClose: () => this.close() }, h("div", { key: '68b0e04fa4d8c35c6523027b630dcd2eb665626a', class: {
                cancel: true,
            } }, h("sc-button", { key: '479d17c9adf9916b94732e6d1a98aef3545ddc00', class: "close__button", type: "text", circle: true, onClick: () => this.close() }, h("sc-icon", { key: '49b2d905743a56ef606ea78dc90c233f4e10a54a', name: "x" })), this.step === 'cancel' && (h("sc-subscription-cancel", { key: 'c18c2ab8ffbd5d67ae85fed2d3cf90b07c23cdb3', subscription: this.subscription, protocol: this.protocol, reason: this.reason, comment: this.comment, onScAbandon: () => this.close(), onScCancelled: () => {
                this.scRefresh.emit();
                this.reset();
                this.scRequestClose.emit('close-button');
            } }, h("slot", { key: 'c74b58911e49b397fa4247456efb8bd747c3e6c1', name: "cancel-popup-content", slot: "cancel-popup-content" }))), this.step === 'survey' && (h("sc-cancel-survey", { key: 'dc2fa0553fa64e95a153a79b678d26146473bebb', protocol: this.protocol, onScAbandon: () => this.close(), onScSubmitReason: e => {
                const { comment, reason } = e.detail;
                this.reason = reason;
                this.comment = comment;
                this.step = (reason === null || reason === void 0 ? void 0 : reason.coupon_enabled) ? 'discount' : 'cancel';
            } })), this.step === 'discount' && (h("sc-cancel-discount", { key: '4f2357a797d2a0de1831a58607bb81768586b610', protocol: this.protocol, subscription: this.subscription, reason: this.reason, comment: this.comment, onScCancel: () => (this.step = 'cancel'), onScPreserved: () => {
                this.scRefresh.emit();
                this.reset();
                this.scRequestClose.emit('close-button');
            } })))));
...
const ScSubscriptionReactivate = class {
    ...
    render() {
        ...
        return (h("sc-dialog", { key: '7996556e1e48336adaa3de4f68aeff76c1286073', noHeader: true, open: this.open, style: { '--width': '600px', '--body-spacing': 'var(--sc-spacing-xxx-large)' } }, h("sc-dashboard-module", { key: '73cbc61afd7958e380ccf8269f6189bc869443ab', loading: this.loading, heading: wp.i18n.__('Resubscribe', 'surecart'), class: "subscription-reactivate", error: this.error, style: { '--sc-dashboard-module-spacing': '1em' } }, this.loading ? (this.renderLoading()) : (h(Fragment, null, h("div", { slot: "description" }, h("sc-alert", { open: true, type: "warning", title: wp.i18n.__('Confirm Charge', 'surecart') }, wp.i18n.__('You will be charged', 'surecart'), " ", (_b = (_a = this.upcomingPeriod) === null || _a === void 0 ? void 0 : _a.checkout) === null || _b === void 0 ? void 0 :
            ...
            } }, wp.i18n.__('Your subscription will be reactivated and will renew automatically on', 'surecart'), " ", h("strong", null, (_c = this.upcomingPeriod) === null || _c === void 0 ? void 0 : _c.end_at_date))), h("sc-flex", { justifyContent: "flex-start" }, h("sc-button", { type: "primary", loading: this.busy, disabled: this.busy, onClick: () => this.reactivateSubscription() }, wp.i18n.__('Yes, Reactivate', 'surecart')), h("sc-button", { disabled: this.busy, style: { color: 'var(--sc-color-gray-500)' }, type: "text", onClick: () => this.scRequestClose.emit() }, wp.i18n.__('No, Keep Inactive', 'surecart'))))), this.busy && h("sc-block-ui", { key: 'e466731ef6e83d3bd02bfce813793ffe3a87dd65' }))));
    }
    static get watchers() { return {
        "open": ["openChanged"]
    }; }
