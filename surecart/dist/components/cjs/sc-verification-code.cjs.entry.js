...
const ScVerificationCode = class {
    ...
    render() {
        return (index.h("div", { key: '74f453ffcdab58d6a165ec948e152989740fc124', class: "sc-verification-code" }, Array.from({ length: this.total }).map((_, index$1) => (index.h(index.Fragment, null, index$1 === Math.floor(this.total / 2) && (index.h("span", { class: "sc-verification-code__separator", "aria-hidden": "true" }, "\u2014")), index.h("input", { key: index$1, id: `code-input-${index$1}`, value: !!this.codes[index$1] ? this.codes[index$1] : '', onInput: e => this.handleInput(e, index$1), onKeyDown: e => this.handleKeyDown(e, index$1), onFocus: e => this.handleFocus(e), autocomplete: "one-time-code", inputmode: "numeric", pattern: "[0-9]*", autofocus: index$1 === 0, required: true, "aria-label": wp.i18n.__(`Verification code ${index$1 + 1} of ${this.total}`, 'surecart') })))), index.h("button", { key: 'e2ed6ab2f8febea2c529c8bec1744adb377a4b11', type: "submit", class: "visually-hidden", onClick: () => this.onChange(this.codes.join('')) }, wp.i18n.__('Submit', 'surecart'))));
    }
